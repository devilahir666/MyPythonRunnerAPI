...
try:
    ...
    # Connection fail hone par server ko exit kar denge
    exit(1)
...
def stream_file(channel_id, message_id):
    ...
    try:
        ...
        def generate():
            # Yahi woh streaming logic hai jo File Size Error ko theek karti hai
    ...
    # Specific error handling for the PEER_ID_INVALID issue
    except (PeerIdInvalid, UserNotParticipant) as e:
        print(f"CRITICAL ACCESS ERROR: {e}. Bot needs to be re-added or meet the peer.")
        # User ko fix karne ka tareeka batao
        return "500 Internal Server Error: Bot has no access. Please forward a file from the channel to the bot and restart the Render service.", 500
